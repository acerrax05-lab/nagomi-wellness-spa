...
function buildServicesChartForecast(labels, serviceCounts) {
  ...
  // Pick background to match color — green for up, red for down, grey for neutral
  ...
  async function updateCompletedStatCard() {
    ...
    insightDiv.innerHTML = `
      <div style="margin-top: 8px; padding: 8px 12px; background: ${isGood ? '#e8f5e9' : '#fff3e0'}; border-radius: 6px; border-left: 3px solid ${isGood ? '#28a745' : '#ff9800'};">
        <div style="font-size: 0.75rem; color: ${isGood ? '#28a745' : '#ff9800'}; font-weight: 700; margin-bottom: 4px;">
  ...
  insightDiv.innerHTML = `
    <div style="margin-top:8px;padding:8px 12px;
      background:${rawPct >= 0 ? '#e8f5e9' : '#fff3e0'};
      border-radius:6px;border-left:3px solid ${color};">
      <div style="font-size:0.75rem;color:${color};font-weight:700;margin-bottom:4px;">
        ${label}
      </div>
              ...
              <div style="padding: 15px; background: ${index === 0 ? '#fff8e1' : '#f5f5f5'}; margin-bottom: 10px; border-radius: 8px; ${index === 0 ? 'border: 2px solid #ffc107;' : ''}">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                  ...
                  <div style="text-align: right;">
                    ...
                    <div style="font-size: 0.8rem; color: #999;">Completion Rate</div>
                  </div>
                </div>
              </div>
            `).join('')}
...
function updateCommissionDisplay(rate) {
  ...
  window.toggleForecast = toggleForecast;
  window.showChartForecast = showChartForecast;
  window.displayChartSpecificForecast = displayChartSpecificForecast;
  ...
  window.highlightBookingRow = highlightBookingRow;
  window.getTherapistDisplay = getTherapistDisplay;
  window.getPreviousPeriodBookings = getPreviousPeriodBookings;
  window.compareToPrevPeriod       = compareToPrevPeriod;
window.buildTodayHourForecast    = buildTodayHourForecast;
window.buildServicesChartForecast = buildServicesChartForecast;

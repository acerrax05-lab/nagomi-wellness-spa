...
(function () {
  ...
  function _showWakeToast() {
    ...
    t.innerHTML = `<span style="font-size:1.2rem;"><i class="fa-regular fa-clock"></i></span>
...
let storeClosures = []; // [{ id, label, start, end }] — single days have start === end
  ...
  socket.on('newBooking', (data) => {
    ...
    addNotif(`<i class="fa-solid fa-calendar-days"></i> New booking: ${data?.guestName || 'Client'} — ${data?.service?.name || ''}`, 'booking', 'bookings');
  ...
  socket.on('booking-cancelled', (data) => {
    addNotif(`<i class="fa-solid fa-circle-xmark"></i> Cancellation request: ${data?.guestName || 'Client'} — ${data?.service?.name || ''}`, 'cancel', 'bookings');
  });

  socket.on('reschedule-request', (data) => {
    addNotif(`<i class="fa-solid fa-rotate"></i> Reschedule request: ${data?.guestName || 'Client'} — ${data?.service?.name || ''}`, 'reschedule', 'bookings');
  });

  socket.on('leave-request', (data) => {
    addNotif(`<i class="fa-solid fa-umbrella-beach"></i> Leave request from ${data?.therapistName || 'a therapist'}`, 'leave', 'leave-requests');
  ...
  document.getElementById('bookingSearchInput')?.addEventListener('input', (e) => {
    ...
    if (searchTerm) {
      ...
      if (infoDiv) infoDiv.innerHTML = `<span style="color:#999;font-size:0.9rem;"><i class="fa-solid fa-magnifying-glass"></i> Searching…</span>`;
  ...
  if (results.length === 0) {
    infoDiv.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <span style="color:#dc3545;font-weight:600;"><i class="fa-solid fa-circle-xmark"></i> No bookings found matching "${searchTerm}"</span>
      </div>`;
  } else {
    infoDiv.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <span style="font-weight:600;color:#2196f3;"><i class="fa-solid fa-circle-check"></i> Found ${results.length} booking${results.length !== 1 ? 's' : ''}</span>
  ...
  if (results.length === 0) {
    tbody.innerHTML = `
      <tr><td colspan="8" style="text-align:center;padding:40px;color:#999;">
        <div style="font-size:3rem;margin-bottom:15px;"><i class="fa-solid fa-magnifying-glass"></i></div>
  ...
  const html = sorted.map((booking, index) => {
    ...
    const genderHtml = (booking.femaleClients > 0 || booking.maleClients > 0)
      ? `<div style="margin-bottom:4px;">
           <span style="font-size:0.78rem;color:#be185d;font-weight:600;"><i class="fa-solid fa-venus"></i>${booking.femaleClients ?? 0}</span>
           <span style="font-size:0.78rem;color:#1d4ed8;font-weight:600;margin-left:4px;"><i class="fa-solid fa-mars"></i>${booking.maleClients ?? 0}</span>
         </div>`
      : '';
    ...
    return `
      <tr data-booking-id="${booking._id}"
        ...
        <td>
          ...
          ${genderHtml}
          <div style="font-size:0.85rem;color:#666;"><i class="fa-solid fa-phone"></i> ${booking.guestPhone || '—'}</div>
          <div style="font-size:0.85rem;color:#666;"><i class="fa-solid fa-ticket"></i> <span style="font-family:monospace;background:#f5f5f5;padding:2px 6px;border-radius:4px;">${booking.transactionNumber || '—'}</span></div>
        </td>
        <td>
  ...
  setTimeout(() => {
    ...
    if (!foundDay) {
      console.warn(' Could not find day in calendar for date:', dateStr);
  ...
  if (!foundRow) {
    console.warn(' Could not find row with booking ID:', bookingId);
  }
}
      ...
      tbody.innerHTML = `
        <tr>
          <td colspan="8" style="text-align:center;padding:40px;color:#999;">
            <div style="font-size:3rem;margin-bottom:15px;"><i class="fa-solid fa-calendar-days"></i></div>
    ...
    if (data.available && data.sarimaEnabled) {
      ...
      indicator.innerHTML = `
        <span style="color:#ff9800;font-weight:600;font-size:0.85rem;">
          <i class="fa-solid fa-triangle-exclamation"></i> Service up but statsmodels missing — run: pip install statsmodels
        </span>`;
    } else {
      indicator.innerHTML = `
        <span style="color:#dc3545;font-weight:600;font-size:0.85rem;">
          <i class="fa-solid fa-gear"></i> JS Fallback (SARIMA offline — run: python sarima_service.py)
        </span>`;
    }
  } catch {
    indicator.innerHTML = `<span style="color:#dc3545;font-size:0.85rem;"><i class="fa-solid fa-gear"></i> JS Fallback active</span>`;
  }
}
...
if (currentPeriod === 'month' || currentPeriod === 'year') {
  ...
  function updateStatLabels() {
    ...
    if (!currentLabels) {
      console.warn(' Invalid period:', currentPeriod);
      return;
    }
...
function buildServicesChartForecast(labels, serviceCounts) {
  ...
  async function displayChartSpecificForecast(chartType, analytics) {
    ...
    const chartTitles = {
      peakHours: '<i class="fa-regular fa-clock"></i> Peak Hours Forecast & Capacity Planning',
      services: '<i class="fa-solid fa-spa"></i> Service Performance & Demand Forecast',
      revenue: '<i class="fa-solid fa-circle-dollar-sign"></i> Revenue Forecast & Trends',
      revenueStatus: '<i class="fa-solid fa-chart-line"></i> Revenue Status & Financial Health',
      bookingDist: '<i class="fa-solid fa-calendar-days"></i> Booking Distribution & Completion Rate',
      predictions: '<i class="fa-solid fa-arrow-trend-up"></i> Booking Predictions & Insights'
    };
        ...
        ${analytics.recommendations && analytics.recommendations.length > 0 ? `
          <div style="margin-top: 40px; padding: 25px; background: linear-gradient(135deg, #fff8e1 0%, #ffe082 30%, #fff8e1 100%); border-radius: 12px; border-left: 4px solid #ff9800;">
            <h3 style="color: #4b2e1e; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
              <span style="font-size: 2rem;"><i class="fa-solid fa-lightbulb"></i></span>
  ...
  function generatePeakHoursInsight(analytics, predictionsData) {
    ...
    return `
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px;">
        <h3 style="margin: 0 0 10px 0; font-size: 1.5rem;"><i class="fa-regular fa-clock"></i> Peak Hours Analysis</h3>
      ...
      <div style="margin-bottom: 30px;">
        <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-arrow-trend-up"></i> Predicted Peak Hours </h4>
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: ${index === 0 ? '#fff8e1' : '#f8f9fa'}; border-radius: 8px; ${index === 0 ? 'border: 2px solid #ffc107;' : ''}">
                <div>
                  <div style="font-weight: 700; color: #4b2e1e;">
                    ${index === 0 ? '🥇 ' : index === 1 ? '<i class="fa-solid fa-medal" style="color:#c0c0c0;"></i> ' : index === 2 ? '<i class="fa-solid fa-medal" style="color:#cd7f32;"></i> ' : '<i class="fa-regular fa-clock"></i> '}${hour}
                  </div>
      ...
      <div style="padding: 20px; background: #e3f2fd; border-radius: 12px; border-left: 4px solid #2196f3;">
        <h4 style="color: #1565c0; margin: 0 0 15px 0;"><i class="fa-solid fa-lightbulb"></i> Capacity Planning Recommendations</h4>
  ...
  function generateServicesInsight(analytics, predictionsData) {
    ...
    return `
      <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px;">
        <h3 style="margin: 0 0 10px 0; font-size: 1.5rem;"><i class="fa-solid fa-spa"></i> Service Performance Analysis</h3>
        <p style="margin: 0; opacity: 0.9; font-size: 0.95rem;">
      ...
      <div style="margin-bottom: 30px;">
        <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-arrow-trend-up"></i> Predicted Top Services </h4>
            ...
            ${overallTopServices.map((service, index) => `
              <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: ${index < 3 ? '#fff8e1' : '#f8f9fa'}; border-radius: 8px; ${index < 3 ? 'border: 2px solid #ffc107;' : ''}">
                <div style="display: flex; align-items: center; gap: 12px;">
                  <span style="font-size: 1.8rem;">${index === 0 ? '🥇' : index === 1 ? '<i class="fa-solid fa-medal" style="color:#c0c0c0;"></i>' : index === 2 ? '<i class="fa-solid fa-medal" style="color:#cd7f32;"></i>' : '<i class="fa-solid fa-star"></i>'}</span>
                  <div>
      ...
      <div style="margin-top: 30px;">
        <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-chart-line"></i> Current Period Performance</h4>
      ...
      <div style="margin-top: 25px; padding: 20px; background: #fff3e0; border-radius: 12px; border-left: 4px solid #ff9800;">
        <h4 style="color: #e65100; margin: 0 0 15px 0;"><i class="fa-solid fa-lightbulb"></i> Service Optimization Tips</h4>
        <ul style="margin: 0; padding-left: 20px; color: #666; line-height: 1.8;">
  ...
  function generateRevenueInsight(analytics, predictionsData) {
    ...
    return `
      <div style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px;">
        <h3 style="margin: 0 0 10px 0; font-size: 1.5rem;"><i class="fa-solid fa-circle-dollar-sign"></i> Revenue Forecast & Analysis</h3>
        <p style="margin: 0; opacity: 0.9; font-size: 0.95rem;">
      ...
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 30px;">
        ...
        <div style="padding: 20px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border-radius: 12px; color: white;">
          ...
          <div style="font-size: 0.85rem; opacity: 0.8;">
            ${revenueGrowth >= 0 ? '<i class="fa-solid fa-arrow-trend-up"></i>' : '<i class="fa-solid fa-arrow-trend-down"></i>'} ${Math.abs(revenueGrowth).toFixed(1)}% vs current period
          </div>
        </div>
      </div>
      ...
      <div style="margin-bottom: 30px;">
        <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-calendar-days"></i> Daily Revenue Forecast</h4>
      ...
      <div style="padding: 20px; background: #e8f5e9; border-radius: 12px; border-left: 4px solid #4caf50;">
        <h4 style="color: #2e7d32; margin: 0 0 15px 0;"><i class="fa-solid fa-lightbulb"></i> Revenue Growth Strategies</h4>
        <ul style="margin: 0; padding-left: 20px; color: #666; line-height: 1.8;">
  ...
  function generateRevenueStatusInsight(analytics, predictionsData) {
    ...
    return `
      <div style="background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px;">
        <h3 style="margin: 0 0 10px 0; font-size: 1.5rem;"><i class="fa-solid fa-chart-line"></i> Revenue Status & Financial Health</h3>
        <p style="margin: 0; opacity: 0.9; font-size: 0.95rem;">
      ...
      <div style="margin-bottom: 30px;">
        <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-arrow-trend-up"></i> Financial Health Metrics</h4>
        <div style="display: grid; gap: 12px;">
  ...
  function generateBookingDistInsight(analytics, predictionsData) {
    ...
    return `
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px;">
        <h3 style="margin: 0 0 10px 0; font-size: 1.5rem;"><i class="fa-solid fa-calendar-days"></i> Booking Distribution & Completion Analysis</h3>
        <p style="margin: 0; opacity: 0.9; font-size: 0.95rem;">
      ...
      <!-- Predicted Bookings -->
      <div style="margin-bottom: 30px;">
        <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-arrow-trend-up"></i> Predicted Bookings </h4>
        ${predictions && predictions.length > 0 ? `
      ...
      <div style="padding: 20px; background: #fff3e0; border-radius: 12px; border-left: 4px solid #ff9800;">
        <h4 style="color: #e65100; margin: 0 0 15px 0;"><i class="fa-solid fa-lightbulb"></i> Performance Insights</h4>
        <ul style="margin: 0; padding-left: 20px; color: #666; line-height: 1.8;">
    ...
    return `
      <div style="margin-top: 40px; padding: 25px; background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%); border-radius: 12px; border-left: 4px solid #2196f3;">
        <h3 style="color: #1565c0; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
          <span style="font-size: 2rem;"><i class="fa-solid fa-chart-line"></i></span>
  ...
  function displayForecastModal(chartType, analytics) {
    ...
    const chartTitles = {
      bookings: '<i class="fa-solid fa-calendar-days"></i> Booking Forecast & Insights',
      revenue: '<i class="fa-solid fa-circle-dollar-sign"></i> Revenue Forecast & Trends',
      services: '<i class="fa-solid fa-spa"></i> Service Performance Analysis',
      peakHours: '<i class="fa-regular fa-clock"></i> Peak Hours & Capacity Planning',
      therapists: '<i class="fa-solid fa-users"></i> Therapist Performance Analytics'
    };
          ...
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 30px;">
            ...
            <div style="padding: 20px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border-radius: 12px; color: white;">
              ...
              <div style="font-size: 2.5rem; font-weight: 700; margin: 10px 0;">
                ${trends.bookings.direction === 'increasing' ? '<i class="fa-solid fa-arrow-trend-up"></i>' : trends.bookings.direction === 'decreasing' ? '<i class="fa-solid fa-arrow-trend-down"></i>' : '<i class="fa-solid fa-arrow-right"></i>'}
              </div>
        ...
        return `
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 30px;">
            ...
            <div style="padding: 20px; background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); border-radius: 12px; color: white;">
              ...
              <div style="font-size: 2.5rem; font-weight: 700; margin: 10px 0;">
                ${trends.revenue.direction === 'increasing' ? '<i class="fa-solid fa-arrow-trend-up"></i>' : trends.revenue.direction === 'decreasing' ? '<i class="fa-solid fa-arrow-trend-down"></i>' : '<i class="fa-solid fa-arrow-right"></i>'}
              </div>
            ...
            ${servicePerformance.slice(0, 10).map((service, idx) => `
              <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: ${idx < 3 ? '#fff8e1' : '#f8f9fa'}; border-radius: 8px; margin-bottom: 10px; ${idx < 3 ? 'border: 2px solid #ffc107;' : ''}">
                <div style="display: flex; align-items: center; gap: 15px;">
                  <span style="font-size: 2rem;">${idx === 0 ? '🥇' : idx === 1 ? '<i class="fa-solid fa-medal" style="color:#c0c0c0;"></i>' : idx === 2 ? '<i class="fa-solid fa-medal" style="color:#cd7f32;"></i>' : '<i class="fa-solid fa-star"></i>'}</span>
                  <div>
                    <div style="font-weight: 700; color: #4b2e1e;">${service.name}</div>
          ...
          <div style="background: white; padding: 25px; border-radius: 12px; margin-bottom: 25px;">
            ...
            ${therapistUtilization.map((therapist, idx) => `
              <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: ${idx < 3 ? '#e8f5e9' : '#f8f9fa'}; border-radius: 8px; margin-bottom: 10px;">
                <div style="display: flex; align-items: center; gap: 15px;">
                  <span style="font-size: 2rem;">${idx === 0 ? '👑' : idx === 1 ? '<i class="fa-solid fa-medal" style="color:#c0c0c0;"></i>' : idx === 2 ? '<i class="fa-solid fa-medal" style="color:#cd7f32;"></i>' : '<i class="fa-solid fa-user"></i>'}</span>
                  <div>
  ...
  function addAnalyticsButtonsToCharts() {
    ...
    chartBoxes.forEach((box) => {
      ...
      button.innerHTML = '<i class="fa-solid fa-chart-line"></i> See Forecast & Insights';
...
//period range
  ...
  if (!ctx) {
    console.log(' predictionsChart canvas not found (not on therapists tab) - skipping chart render');
    return;
  }
  ...
  if (currentChart.predictions) {
    ...
    if (!predictions || predictions.length === 0) {
      console.warn(' No predictions data to display');
      return;
    }
    ...
    currentChart.predictions = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: `<i class="fa-solid fa-arrow-trend-up"></i> Booking Predictions (${forecastHorizon})`,
            font: {
              size: 14,
              weight: 'bold'
            },
  ...
  function addForecastIndicator(chartContainer) {
    ...
    indicator.textContent = '<i class="fa-solid fa-chart-line"></i> Forecast Active';
  ...
  async function showComprehensiveInsights() {
    ...
    title.textContent = '<i class="fa-solid fa-chart-line"></i> Forecast & Prescription Insights';
    ...
    try {
      ...
      content.innerHTML = `
        <div style="padding: 40px; text-align: center;">
          <div style="font-size: 3rem; margin-bottom: 15px;"><i class="fa-solid fa-triangle-exclamation"></i></div>
  ...
  function displayComprehensiveInsightsContent(predictionsData, analyticsData) {
    ...
    content.innerHTML = `
      <div style="padding: 20px;">
        <!-- Header -->
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px;">
          <h3 style="margin: 0 0 10px 0; font-size: 1.5rem;"><i class="fa-solid fa-chart-line"></i> Forecast Period: ${forecastHorizon}</h3>
          <p style="margin: 0; opacity: 0.9; font-size: 0.95rem;">
        ...
        <div class="forecast-comparison-grid">
          ...
          <div class="forecast-comparison-card">
            ...
            <div class="forecast-comparison-value" style="font-size: 1.5rem;">
              ${predictions.filter(p => p.confidence === 'High').length > predictions.length / 2 ? 'High <i class="fa-solid fa-circle" style="color:#28a745;"></i>' : 'Medium <i class="fa-solid fa-circle" style="color:#ffc107;"></i>'}
            </div>
          </div>
        </div>
        ...
        <div style="margin-top: 35px;">
          <h4 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-calendar-days"></i> Detailed Forecast Timeline</h4>
        ...
        <div style="margin-top: 25px; padding: 20px; background: #e3f2fd; border-radius: 12px; border-left: 4px solid #2196f3;">
          <div style="display: flex; align-items: start; gap: 15px;">
            <div style="font-size: 2rem;"><i class="fa-solid fa-circle-info"></i></div>
            <div>
              ...
              <p style="color: #666; margin: 10px 0 0 0; line-height: 1.6; font-size: 0.85rem; font-style: italic;">
                <i class="fa-solid fa-lightbulb"></i> Use the forecast toggle in the main dashboard to enable/disable predictions on charts.
              </p>
            </div>
          </div>
        </div>
  ...
  function calculateStats(bookings) {
    ...
    // <i class="fa-solid fa-circle-check"></i> ADD INLINE INSIGHTS
 ...
 function addChartNotification(chartId, actualData, predictedData, type) {
  ...
  if (!forecastEnabled) {
    ...
    notification.innerHTML = `
      <span style="font-size:1.4rem;">${isCurrency ? '<i class="fa-solid fa-circle-dollar-sign"></i>' : '<i class="fa-solid fa-calendar-days"></i>'}</span>
      <div>
  ...
  if (actualTotal === 0) {
    notification.innerHTML = `
      <span style="font-size:1.4rem;"><i class="fa-solid fa-arrow-trend-up"></i></span>
      <div>
  ...
  if (overlapSlots === 0) {
    ...
    const trend = forecastAvg >= actualAvg ? '<i class="fa-solid fa-arrow-trend-up"></i>' : '<i class="fa-solid fa-arrow-trend-down"></i>';
  ...
  const icon  = rawPct >  2 ? '<i class="fa-solid fa-arrow-trend-up"></i>' : rawPct < -2 ? '<i class="fa-solid fa-arrow-trend-down"></i>' : '<i class="fa-solid fa-arrow-right"></i>';
    ...
    options: {
      ...
      plugins: { 
        ...
        tooltip: {
          ...
          callbacks: {
            title: function(context) {
              return `${currentPeriod === 'today' ? '<i class="fa-regular fa-clock"></i>' : '<i class="fa-solid fa-calendar-days"></i>'} ${context[0].label}`;
            },
            label: function(context) {
...
console.log(' Services chart - bookings:', bookings.length, 'unique services:', Object.keys(serviceCounts).length, serviceCounts);
  ...
  // <i class="fa-solid fa-circle-check"></i> DECLARE forecastRevenue HERE
  ...
  // <i class="fa-solid fa-circle-check"></i> BUILD DATASETS
  const datasets = [
    {
  ...
  // <i class="fa-solid fa-circle-check"></i> ADD FORECAST if enabled
  // <i class="fa-solid fa-circle-check"></i> ADD FORECAST — aggregate per period so daily predictions
      ...
      plugins: { 
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            // <i class="fa-solid fa-circle-dollar-sign"></i> LABEL: Show revenue with insight
            label: function(context) {
              const datasetLabel = context.dataset.label;
              const value = context.parsed.y;
            ...
            // <i class="fa-solid fa-chart-line"></i> FOOTER: Show additional insights
            footer: function(context) {
              ...
              if (context.length > 1 && context[1].dataset.label === 'Predicted Revenue') {
                ...
                if (Math.abs(percentDiff) > 5) {
                  const status = difference > 0 ? 
                    `<i class="fa-solid fa-circle-check"></i> Exceeding forecast by ${percentDiff}%` :
                    `<i class="fa-solid fa-triangle-exclamation"></i> Below forecast by ${Math.abs(percentDiff)}%`;
                  return `\n${status}`;
                }
              }
              ...
              if (relevantBookings > 0) {
                return `\n<i class="fa-solid fa-calendar-days"></i> ${relevantBookings} booking${relevantBookings !== 1 ? 's' : ''} completed`;
              }
  ...
  // <i class="fa-solid fa-circle-check"></i> ADD CHART NOTIFICATION (NOW forecastRevenue is in scope!)
  ...
  const chartBox = document.getElementById('revenueStatusChart')?.closest('.chart-box');
    if (chartBox) {
      ...
      const icon = growthRate > 0 ? '<i class="fa-solid fa-arrow-trend-up"></i>' : growthRate < 0 ? '<i class="fa-solid fa-arrow-trend-down"></i>' : '<i class="fa-solid fa-arrow-right"></i>';
...
function renderPendingRequestCard(booking) {
  ...
  const typeLabel  = isCancellation ? '<i class="fa-solid fa-circle-xmark"></i> Cancellation Request' : '<i class="fa-solid fa-calendar-days"></i> Reschedule Request';
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <div style="color:#666;font-size:0.88rem;margin-top:4px;">
            <i class="fa-solid fa-phone"></i> ${booking.guestPhone} &nbsp;|&nbsp;
            <i class="fa-solid fa-ticket"></i> <span style="font-family:monospace;background:#f5f5f5;padding:2px 8px;border-radius:4px;">
        ...
        <div style="display:flex;gap:10px;flex-shrink:0;">
          ${isCancellation ? `
            <button
              ...
              <i class="fa-solid fa-circle-check"></i> Approve Cancellation
            </button>
            <button
          ...
          ` : `
            <button
              ...
              onmouseover="this.style.background='#1e7e34'"
              onmouseout="this.style.background='#28a745'">
              <i class="fa-solid fa-circle-check"></i> Approve Reschedule
            </button>
            <button
...
async function approveCancellation(bookingId) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) { showNotification(`<i class="fa-solid fa-circle-xmark"></i> ${data.msg}`, 'error'); return; }

    showNotification('<i class="fa-solid fa-circle-check"></i> Cancellation approved. Booking has been cancelled.', 'success');
    loadPendingRequests();
    if (allBookings) {
  ...
  if (idx > -1) allBookings[idx].status = 'cancelled';
    ...
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Error approving cancellation', 'error');
  }
}
...
async function rejectCancellation(bookingId) {
  ...
  try {
    ...
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Cancellation request rejected. Booking remains active.', 'success');
    loadPendingRequests();
    if (selectedDate) loadBookingsForDate(selectedDate);

  } catch (err) {
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Error rejecting cancellation', 'error');
  }
}
...
async function approveReschedule(bookingId) {
  ...
  try {
    ...
    showNotification('<i class="fa-solid fa-circle-check"></i> Reschedule approved. Booking has been moved.', 'success');
    loadPendingRequests();
    if (allBookings) {
  const idx = allBookings.findIndex(b => b._id === bookingId);
  if (idx > -1) allBookings[idx].status = 'cancelled';
    ...
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Error approving reschedule', 'error');
  }
}
...
async function rejectReschedule(bookingId) {
  ...
  try {
    ...
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Reschedule request rejected. Booking stays at original time.', 'success');
    loadPendingRequests();
    if (selectedDate) loadBookingsForDate(selectedDate);

  } catch (err) {
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Error rejecting reschedule', 'error');
  }
}
    ...
    return `
      <div style="display:flex;flex-direction:column;gap:6px;">
        <button class="btn-confirm"
          ...
          style="background:#28a745;"><i class="fa-solid fa-circle-check"></i> Approve Cancel</button>
        <button class="btn-cancel-booking"
          onclick="rejectCancellation('${b._id}')"
          style="background:#dc3545;"><i class="fa-solid fa-xmark"></i> Reject</button>
      </div>`;
  }
    ...
    return `
      <div style="display:flex;flex-direction:column;gap:6px;">
        <button class="btn-confirm"
          ...
          style="background:#28a745;"><i class="fa-solid fa-circle-check"></i> Approve Reschedule</button>
        <button class="btn-cancel-booking"
...
window.getBookingRowActions = getBookingRowActions;
  ...
  dayCell.innerHTML = `
    ...
    ${isBlocked ? `<span class="closure-badge">${isSingle ? '<i class="fa-solid fa-ban"></i>' : '<i class="fa-solid fa-umbrella-beach"></i>'} ${closure.label || 'Closed'}</span>` : ''}
  `;
  ...
  function loadBookingsForDate(dateStr) {
    ...
    // <i class="fa-solid fa-circle-check"></i> Separate walk-in and online bookings
    ...
    // <i class="fa-solid fa-circle-check"></i> Display Online Bookings First
  ...
  <i class="fa-solid fa-globe"></i> ONLINE BOOKINGS (${onlineBookings.length})
</td>
        </tr>
      `;
      ...
      if (mobileList) {
        mobileList.innerHTML += `<div class="mobile-group-header mobile-group-online"><i class="fa-solid fa-globe"></i> ONLINE BOOKINGS (${onlineBookings.length})</div>`;
        mobileList.innerHTML += onlineBookings.map(b => generateMobileBookingCard(b, 'online')).join('');
      }
    }

    // <i class="fa-solid fa-circle-check"></i> Display Walk-in Bookings
  ...
  <i class="fa-solid fa-person-walking"></i> WALK-IN BOOKINGS (${walkInBookings.length})
</td>
        </tr>
      `;
      ...
      if (mobileList) {
        mobileList.innerHTML += `<div class="mobile-group-header mobile-group-walkin"><i class="fa-solid fa-person-walking"></i> WALK-IN BOOKINGS (${walkInBookings.length})</div>`;
  ...
  function generateMobileBookingCard(b, type) {
    ...
    const genderHtml  = (femaleCount !== null && maleCount !== null)
      ? `<span class="mc-gender mc-female"><i class="fa-solid fa-venus"></i>${femaleCount}</span><span class="mc-gender mc-male"><i class="fa-solid fa-mars"></i>${maleCount}</span>`
      : '';
    ...
    const typeBadge = type === 'walk-in'
      ? `<span class="mc-type-badge mc-walkin"><i class="fa-solid fa-person-walking"></i> WALK-IN</span>`
      : `<span class="mc-type-badge mc-online"><i class="fa-solid fa-globe"></i> ONLINE</span>`;
    ...
    return `
      <div class="mobile-booking-card" data-booking-id="${b._id}">
        ...
        <div class="mc-time-service">
          <span class="mc-time-pill"><i class="fa-regular fa-clock"></i> ${startTime}${endTime !== '—' ? ` – ${endTime}` : ''}</span>
          <span class="mc-service">${serviceName}</span>
        </div>
        <div class="mc-meta">
          <span><i class="fa-solid fa-user"></i> ${therapistName}</span>
        ...
        <div class="mc-contact">
          ...
          <span class="mc-sep">·</span>
          <span class="mc-txn"><i class="fa-solid fa-ticket"></i> ${txn}</span>
        </div>
        <div style="margin:5px 0 3px;">
          <button class="mc-reassign-btn" onclick="openAssignTherapist('${b._id}', '${therapistName.replace(/'/g,"\\'")}')"><i class="fa-solid fa-user"></i> Reassign</button>
...
function formatBookingDate(b) {
  ...
  const genderBreakdown = (femaleCount !== null && maleCount !== null && (femaleCount > 0 || maleCount > 0))
    ? `<span style="font-size:0.78rem;color:#be185d;font-weight:600;"><i class="fa-solid fa-venus"></i>${femaleCount}</span>
       <span style="font-size:0.78rem;color:#1d4ed8;font-weight:600;margin-left:4px;"><i class="fa-solid fa-mars"></i>${maleCount}</span>`
    : '';
  const startTime  = b.time || '—';
  const endTime    = formatEndTime(b.endTime);
  ...
  const typeBadge = type === 'walk-in'
    ? `<span style="display:inline-block;padding:4px 8px;background:#fff3e0;color:#f57c00;
        ...
        <i class="fa-solid fa-person-walking"></i> WALK-IN</span>`
    : `<span style="display:inline-block;padding:4px 8px;background:#e3f2fd;color:#1976d2;
        ...
        <i class="fa-solid fa-globe"></i> ONLINE</span>`;
  ...
  return `
    <tr style="${type === 'walk-in' ? 'background:#fffbf0;' : ''}" data-booking-id="${b._id}">
      <td style="word-wrap:break-word;overflow-wrap:break-word;">
        ...
        <div style="font-size:0.85rem;color:#666;margin-top:4px;"><i class="fa-solid fa-phone"></i> ${b.guestPhone || 'No phone'}</div>
        <div style="font-size:0.85rem;color:#666;">
          <i class="fa-solid fa-ticket"></i> <span style="font-family:monospace;background:#f5f5f5;padding:2px 8px;border-radius:4px;font-size:0.75rem;">
      ...
      <td>${(() => {
        const statusLabels = {
          ...
          pending_reschedule:   { label: 'Reschedule <i class="fa-regular fa-clock"></i>', cls: 'pending_reschedule' },
          pending_cancellation: { label: 'Cancel Req <i class="fa-regular fa-clock"></i>', cls: 'pending_cancellation' },
        };
    ...
    try {
      ...
      modal.innerHTML = `
        <div style="background: white; padding: 30px; border-radius: 16px; max-width: 500px; width: 90%; max-height: 80vh; overflow-y: auto;">
          <h3 style="color: #4b2e1e; margin-bottom: 20px;"><i class="fa-solid fa-circle-check"></i> Available Therapists (${available.length})</h3>
  ...
  async function assignTherapist(bookingId, therapistId) {
    try {
      ...
      if (res.ok) {
        showNotification('<i class="fa-solid fa-circle-check"></i> Therapist assigned successfully!', 'success');
        ...
        const data = await res.json();
        alert(`<i class="fa-solid fa-circle-xmark"></i> ${data.msg || 'Failed to assign therapist'}`);
      }
    } catch (err) {
      console.error(err);
  ...
  return `
    <td>
      ...
      <button 
        ...
        <i class="fa-solid fa-user"></i> Assign Therapist
      </button>
    </td>
  `;
}
...
async function saveClosures() {
  try {
    ...
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Failed to save closures', 'error');
    throw e;
  }
}
  ...
  panel.innerHTML = `
    ...
    ">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
        <h3 style="margin:0;color:#4b2e1e;display:flex;align-items:center;gap:8px;font-size:1.1rem;">
          <i class="fa-solid fa-ban"></i> Store Closures
        ...
        <div style="display:grid;gap:8px;">
          ...
          ${vacations.length > 0 ? `
            <div style="font-size:0.78rem;font-weight:700;color:#888;text-transform:uppercase;
              letter-spacing:0.06em;margin-bottom:2px;"><i class="fa-solid fa-umbrella-beach"></i> Vacation / Multi-day Closures</div>
          ...
          ${holidays.length > 0 ? `
            <div style="font-size:0.78rem;font-weight:700;color:#888;text-transform:uppercase;
              letter-spacing:0.06em;margin-bottom:2px;"><i class="fa-solid fa-ban"></i> Single-day Holidays</div>
    ...
    <div id="holidayModal" style="display:none;position:fixed;inset:0;
      ...
      <div style="background:white;padding:32px;border-radius:16px;width:380px;
        ...
        <div id="holidayModalIcon" style="font-size:3rem;margin-bottom:12px;"><i class="fa-solid fa-ban"></i></div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;
      ...
      <div>
        ...
        <div style="color:#666;font-size:0.83rem;margin-top:2px;">
          ${isSingle
            ? `<i class="fa-solid fa-calendar-days"></i> ${startFmt}`
            : `<i class="fa-solid fa-calendar-days"></i> ${startFmt} → ${endFmt}
...
async function confirmAddClosure() {
  ...
  try {
    ...
    const days = Math.round((new Date(end) - new Date(start)) / 86400000) + 1;
    showNotification(
      `<i class="fa-solid fa-ban"></i> "${label}" saved — ${days} day${days !== 1 ? 's' : ''} blocked`,
      'success'
    );
    closeClosureModal();
    renderClosurePanel();
...
async function deleteClosure(id) {
  ...
  try {
    await saveClosures();
    showNotification('<i class="fa-solid fa-circle-check"></i> Closure removed', 'success');
    renderClosurePanel();
    renderCalendar();
  } catch (e) {}
}
...
function openHolidayModal(dateStr, isCurrentlyHoliday) {
  ...
  document.getElementById('holidayModalIcon').textContent  = isCurrentlyHoliday ? '<i class="fa-solid fa-circle-check"></i>' : '<i class="fa-solid fa-ban"></i>';
...
async function confirmHolidayToggle() {
  if (_holidayIsRemoving) {
    ...
    showNotification('<i class="fa-solid fa-circle-check"></i> Holiday removed', 'success');
  } else {
    ...
    showNotification(`<i class="fa-solid fa-ban"></i> "${labelVal}" marked as holiday`, 'success');
  }
...
async function confirmAssignTherapist() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      showNotification('<i class="fa-solid fa-circle-xmark"></i> Failed to assign therapist', 'error');
    }
  } catch (e) {
    showNotification('<i class="fa-solid fa-circle-xmark"></i> Server error', 'error');
  }
}
      ...
      const imgThumb = s.image
        ...
        : `<div class="svc-img-thumb" style="
            ...
            <span style="font-size:0.68rem;color:#b09070;text-align:center;line-height:1.3;padding:4px;">
              <i class="fa-solid fa-camera"></i><br>No image
            </span>
          </div>`;
        ...
        <div class="service-card" style="${isHidden ? 'opacity:0.55;' : ''}display:flex;align-items:flex-start;gap:14px;">
          ...
          <div class="service-info" style="flex:1;min-width:0;">
            ...
            <p style="color:#666;font-size:0.85rem;margin-top:8px;font-style:italic;">
              <i class="fa-solid fa-circle-info"></i> ${durationsText}
            </p>
          </div>
          <div class="service-actions">
            ...
            >${isHidden ? '<i class="fa-solid fa-eye"></i> Show' : '<i class="fa-solid fa-ban"></i> Hide'}</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  async function toggleServiceVisibility(serviceId, currentlyHidden) {
    ...
    try {
      ...
      if (res.ok) {
        showNotification(
          currentlyHidden ? '<i class="fa-solid fa-circle-check"></i> Service is now visible' : '<i class="fa-solid fa-ban"></i> Service hidden from booking page',
          'success'
        );
        loadServices();
      } else {
  ...
  function injectServiceImagePanel(serviceId, currentImageUrl) {
    ...
    panel.innerHTML = `
      <p style="font-weight:600;color:#4b2e1e;margin:0 0 10px;font-size:0.9rem;">
        <i class="fa-solid fa-camera"></i> Service Image
      </p>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ...
        " onmouseover="this.style.background='#a07850'"
          ...
          <i class="fa-solid fa-folder"></i> ${hasImg ? 'Change Image' : 'Upload Image'}
    ...
    fileInput.addEventListener('change', async () => {
      ...
      if (file.size > 5 * 1024 * 1024) {
        showImgStatus('<i class="fa-solid fa-circle-xmark"></i> File too large. Max size is 5 MB.', 'error');
        return;
      }
      ...
      try {
        ...
        panel.querySelector('#svcImgUploadBtn').querySelector('span') && 
          (panel.querySelector('#svcImgUploadBtn').childNodes[0].textContent = '<i class="fa-solid fa-folder"></i> Change Image');

        showImgStatus('<i class="fa-solid fa-circle-check"></i> Image uploaded successfully!', 'success');
        ...
        showImgStatus(`<i class="fa-solid fa-circle-xmark"></i> ${err.message}`, 'error');
      }
    ...
    panel.querySelector('#svcImgRemoveBtn').addEventListener('click', async () => {
      ...
      try {
        ...
        showImgStatus('<i class="fa-solid fa-trash"></i> Image removed.', 'success');
        loadServices();
      } catch (err) {
        showImgStatus(`<i class="fa-solid fa-circle-xmark"></i> ${err.message}`, 'error');
      }
  ...
  document.getElementById('serviceForm').addEventListener('submit', async (e) => {
    ...
    if (allowedDurations.length === 0) {
      alert('<i class="fa-solid fa-circle-xmark"></i> Please select at least one duration');
      return;
    }
      ...
      therapistsList.innerHTML = therapists.map(t => {
        ...
        return `
          <div class="therapist-card">
            <div class="therapist-avatar"><i class="fa-solid fa-user"></i></div>
            <h3>${t.name}</h3>
            <p>${t.email}</p>
            <p style="margin-top:6px;">
              <span style="display:inline-block;padding:2px 10px;border-radius:12px;font-size:0.78rem;font-weight:700;
                ...
                ${t.gender === 'male' ? '<i class="fa-solid fa-mars"></i> Male' : '<i class="fa-solid fa-venus"></i> Female'}
              </span>
            </p>
            ...
            <div class="service-actions" style="justify-content: center; margin-top: 16px; flex-direction: column; gap: 10px;">
              <button class="btn-schedule" onclick="openScheduleModal('${t._id}')">
                <i class="fa-solid fa-calendar-days"></i> Manage Schedule
              </button>
              <div style="display: flex; gap: 8px;">
                <button class="btn-edit" onclick="editTherapist('${t._id}')">Edit</button>
                <button class="btn-archive" onclick="archiveTherapist('${t._id}', '${t.name}')"><i class="fa-solid fa-box-archive"></i> Archive</button>
              </div>
            </div>
          </div>
        `;
        ...
        <div class="day-schedule" data-day="${day}">
          ...
          <div class="shifts-container" id="shifts-${day}" style="display: ${daySchedule.isWorking ? 'flex' : 'none'}; flex-direction: column; gap: 10px;">
            <h5 style="width: 100%; margin: 10px 0 8px 0 !important; color: #4b2e1e; font-size: 1rem !important; font-weight: 600; padding-bottom: 5px; border-bottom: 1px solid #e0e0e0;">
              <i class="fa-solid fa-briefcase"></i> Working Hours
            </h5>
            ...
            <h5 style="width: 100%; margin: 20px 0 8px 0 !important; color: #ff9800; font-size: 1rem !important; font-weight: 700; padding: 8px 12px; background: #fff3e0; border-radius: 6px; border-left: 4px solid #ff9800;">
              <i class="fa-regular fa-clock"></i> Break Times
            </h5>
            <div class="breaks-container" id="breaks-${day}">
              ${breaksHTML}
            </div>
            <button class="btn-add-break" onclick="addBreak('${day}')" type="button"><i class="fa-regular fa-clock"></i> Add Break</button>
          </div>
        </div>
      `;
    }).join('');
      ...
      return `
        <div class="override-item ${override.isWorking ? 'working' : ''} ${isPast ? 'past' : ''}">
          <div class="override-details">
            ...
            <p><strong>Status:</strong> ${override.isWorking ? '<i class="fa-solid fa-circle-check"></i> Working (Custom Hours)' : '<i class="fa-solid fa-circle-xmark"></i> Not Working'}</p>
  ...
  async function saveTherapistSchedule() {
    try {
      ...
      showNotification('<i class="fa-solid fa-circle-check"></i> Schedule updated successfully!', 'success');
      closeScheduleModal();
      loadTherapists();
    ...
    if (archives.length === 0) {
      container.innerHTML = `
        <div style="text-align:center; padding:60px; color:#999;">
          <div style="font-size:3rem; margin-bottom:12px;"><i class="fa-solid fa-box-archive"></i></div>
        ...
        return `
          <div class="archived-therapist-card">
            <div class="archived-card-header">
              <div class="archived-avatar"><i class="fa-solid fa-user"></i></div>
              <div>
                <h3>${t.name}</h3>
              ...
              <button class="btn-delete" style="margin-left:auto; align-self:flex-start; font-size:0.8rem; padding:4px 10px;"
                onclick="permanentlyDeleteArchive(${realIdx})"><i class="fa-solid fa-trash"></i> Remove</button>
              <button class="btn-schedule" style="align-self:flex-start; font-size:0.8rem; padding:4px 10px; margin-left:6px;"
                onclick="unarchiveTherapist(${realIdx})"><i class="fa-solid fa-recycle"></i> Unarchive</button>
            </div>
            <div class="archived-reason-box">
              <strong><i class="fa-solid fa-clipboard-list"></i> Reason for Archive:</strong> ${t.archiveReason}
            </div>
        ...
        <div class="grace-day-section" data-day="${day}">
          <div class="grace-day-header">
            ...
            <button class="btn-add-grace" onclick="addGracePeriod('${day}')">
              <i class="fa-regular fa-clock"></i> Add Rest Period
            </button>
          </div>
    ...
    container.innerHTML = `
      <div class="post-service-rest-box">
        <div class="psr-header">
          <div>
            <h4><i class="fa-regular fa-clock"></i> Post-Service Rest Period</h4>
  ...
  async function saveGracePeriods() {
    try {
      ...
      if (res.ok) {
        showNotification('<i class="fa-solid fa-circle-check"></i> Grace periods saved successfully!', 'success');
        gracePeriodsData = updatedData;
      } else {
        const data = await res.json();
        showNotification(`<i class="fa-solid fa-circle-xmark"></i> ${data.msg || 'Failed to save'}`, 'error');
      }
    } catch (err) {
  ...
  async function savePostServiceRest() {
    try {
      ...
      if (res.ok) {
        ...
        showNotification('<i class="fa-solid fa-circle-check"></i> Post-service rest period updated!', 'success');
        renderPostServiceRest();
      } else {
        const data = await res.json();
        showNotification(`<i class="fa-solid fa-circle-xmark"></i> ${data.msg || 'Failed to save'}`, 'error');
  ...
  function displayTherapistPerformance(analytics) {
    const grid = document.getElementById('therapistStatusGrid');
    if (!grid) { console.warn(' therapistStatusGrid element not found'); return; }
    ...
    const statusMeta = {
      available: { icon: '<i class="fa-solid fa-circle" style="color:#28a745;"></i>', cls: 'available', label: 'Available Now' },
      busy:      { icon: '<i class="fa-solid fa-circle" style="color:#dc3545;"></i>', cls: 'busy',      label: 'In Session'    },
      break:     { icon: '<i class="fa-solid fa-circle" style="color:#ffc107;"></i>', cls: 'break',     label: 'On Break'      },
      off:       { icon: '<i class="fa-solid fa-circle" style="color:#6c757d;"></i>', cls: 'off',        label: 'Off Duty'      },
    };

    grid.innerHTML = analytics.map(t => {
      const sm = statusMeta[t.status] || { icon: '<i class="fa-solid fa-circle" style="color:#6c757d;"></i>', cls: 'off', label: t.statusMessage || 'Unknown' };
      const expertiseTxt = t.expertise && t.expertise.length > 0
        ? t.expertise.slice(0, 2).join(', ') + (t.expertise.length > 2 ? ` +${t.expertise.length - 2}` : '')
        : 'All services';
      const genderBadge = `<span style="display:inline-block;padding:2px 10px;border-radius:12px;font-size:0.76rem;font-weight:700;background:${t.gender === 'male' ? '#dbeafe' : '#fce7f3'};color:${t.gender === 'male' ? '#1d4ed8' : '#be185d'};">${t.gender === 'male' ? '<i class="fa-solid fa-mars"></i> Male' : '<i class="fa-solid fa-venus"></i> Female'}</span>`;
      ...
      return `
        <div class="therapist-status-card ${sm.cls}" style="display:flex;flex-direction:column;">
          ...
          <div style="display:flex;align-items:center;gap:14px;padding:16px 18px 12px;">
            <div style="width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#8b6f47,#4b2e1e);color:#fff;font-size:1.5rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><i class="fa-solid fa-user"></i></div>
          ...
          <div style="padding:10px 18px 14px;border-top:1px solid rgba(0,0,0,0.06);display:flex;flex-direction:column;gap:8px;">
            <button class="btn-schedule" style="width:100%;" onclick="openScheduleModal('${tid}')"><i class="fa-solid fa-calendar-days"></i> Manage Schedule</button>
            <div style="display:flex;gap:8px;">
              <button class="btn-edit" style="flex:1;" onclick="editTherapist('${tid}')">Edit</button>
              <button class="btn-archive" style="flex:1;" onclick="archiveTherapist('${tid}', '${safeName}')"><i class="fa-solid fa-box-archive"></i> Archive</button>
            </div>
          </div>
  ...
  try {
    ...
    showNotification(`<i class="fa-solid fa-circle-check"></i> Commission rate saved: ${rate}%`, 'success');

  } catch (err) {
    console.error('saveCommissionSettings error:', err);
    showNotification(`<i class="fa-solid fa-circle-xmark"></i> Failed to save: ${err.message}`, 'error');
  }
}
...
async function loadCommissionSettings() {
    try {
      ...
      if (res.ok) {
        ...
        console.log(' Commission settings not found, using defaults');
...
function updateCommissionDisplay(rate) {
    ...
    grid.innerHTML = statusData.map(therapist => {
      ...
      switch (status) {
        case 'available':
          ...
          statusIcon = '<i class="fa-solid fa-circle" style="color:#28a745;"></i>';
          break;
        case 'busy':
          statusText = 'In Session';
          statusIcon = '<i class="fa-solid fa-circle" style="color:#dc3545;"></i>';
          break;
        case 'break':
          statusText = 'On Break';
          statusIcon = '<i class="fa-solid fa-circle" style="color:#ffc107;"></i>';
          break;
        case 'off':
          statusText = 'Off Duty';
          statusIcon = '<i class="fa-solid fa-circle" style="color:#6c757d;"></i>';
          break;
      }
  ...
  function displayIncomeTable(incomeData) {
    ...
    tbody.innerHTML = incomeData.map((therapist, index) => {
      ...
      const medal = index === 0 ? '<i class="fa-solid fa-trophy"></i> ' : index === 1 ? '<i class="fa-solid fa-medal" style="color:#c0c0c0;"></i> ' : index === 2 ? '<i class="fa-solid fa-medal" style="color:#cd7f32;"></i> ' : '';
      ...
      if (container) {
        container.innerHTML = `
          <div style="padding: 60px; text-align: center;">
            <div style="font-size: 4rem; margin-bottom: 20px;"><i class="fa-solid fa-triangle-exclamation"></i></div>
  ...
  container.innerHTML = `
    ...
    <div class="review-filters" style="margin-bottom: 20px; display: flex; gap: 10px; flex-wrap: wrap;">
      ...
      <button class="filter-btn" onclick="filterReviews(5)">5 <i class="fa-solid fa-star"></i> (${reviews.filter(r=>r.rating===5).length})</button>
      <button class="filter-btn" onclick="filterReviews(4)">4 <i class="fa-solid fa-star"></i> (${reviews.filter(r=>r.rating===4).length})</button>
      <button class="filter-btn" onclick="filterReviews(3)">3 <i class="fa-solid fa-star"></i> (${reviews.filter(r=>r.rating===3).length})</button>
      <button class="filter-btn" onclick="filterReviews(2)">2 <i class="fa-solid fa-star"></i> (${reviews.filter(r=>r.rating===2).length})</button>
      <button class="filter-btn" onclick="filterReviews(1)">1 <i class="fa-solid fa-star"></i> (${reviews.filter(r=>r.rating===1).length})</button>
    </div>

    <div id="reviewsList" style="display:flex; flex-direction:column; gap:16px; width:100%;">
      ${reviews.length === 0
        ? `<div style="padding:60px;text-align:center;"><div style="font-size:4rem;"><i class="fa-solid fa-star-half-stroke"></i></div><h3>No Reviews Yet</h3></div>`
  ...
  window.openBookingExportModal = function() {
    ...
    m.innerHTML = `
      <div class="modal-content" style="max-width:480px;text-align:left;">
        <h3 style="color:#4b2e1e;margin-bottom:6px;"><i class="fa-solid fa-chart-line"></i> Export Bookings to Excel</h3>
  ...
  window.runBookingExport = function() {
    ...
    showNotification(`<i class="fa-solid fa-circle-check"></i> Exported ${filtered.length} bookings to Excel!`, 'success');
  ...
  function exportIncomeReport() {
    ...
    if (isModalOpen && _detailTherapistId && _detailBookings) {
      ...
      showNotification(`<i class="fa-solid fa-circle-check"></i> Exported ${filtered.length} records for ${_detailTherapistName}!`, 'success');
      return;
    }
    ...
    showNotification(`<i class="fa-solid fa-circle-check"></i> Exported ${filtered.length} records to Excel!`, 'success');
  }
...
function addNotif(message, type = 'booking', targetTab = null) {
  const icons = { booking: '<i class="fa-solid fa-calendar-days"></i>', cancel: '<i class="fa-solid fa-circle-xmark"></i>', reschedule: '<i class="fa-solid fa-rotate"></i>', leave: '<i class="fa-solid fa-umbrella-beach"></i>', general: '<i class="fa-solid fa-circle-info"></i>' };
  notifStore.unshift({ message, type, icon: icons[type] || '<i class="fa-solid fa-bell"></i>', time: new Date(), read: false, targetTab });
...
async function switchArchiveTab(tab) {
  ...
  if (tab === 'active') {
    ...
    try {
      ...
      list.innerHTML = active.map(t => `
        <div class="archive-list-row">
          <div class="archive-row-avatar"><i class="fa-solid fa-user"></i></div>
    ...
    list.innerHTML = archives.map((t, i) => {
      ...
      return `
        <div class="archive-list-row">
          <div class="archive-row-avatar" style="opacity:0.6;"><i class="fa-solid fa-user"></i></div>
          <div class="archive-row-info">
          ...
          <div style="display:flex;gap:6px;flex-shrink:0;">
            <button class="btn-schedule" style="font-size:0.78rem;padding:5px 10px;"
              onclick="unarchiveTherapist(${realIdx});closeManageArchiveModal();"><i class="fa-solid fa-recycle"></i> Restore</button>
            <button class="btn-delete" style="font-size:0.78rem;padding:5px 10px;"
              onclick="permanentlyDeleteArchive(${realIdx});switchArchiveTab('archived');"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>`;
    }).join('');
  }
...
async function loadLeaveRequests() {
  ...
  try {
    ...
    if (!requests || requests.length === 0) {
      container.innerHTML = `
        <div style="text-align:center;padding:60px;color:#999;">
          <div style="font-size:3rem;margin-bottom:12px;"><i class="fa-solid fa-umbrella-beach"></i></div>
    ...
    if (pending.length > 0) {
      container.innerHTML += `<h3 style="color:#4b2e1e;font-family:'Playfair Display',serif;margin-bottom:12px;"><i class="fa-regular fa-clock"></i> Pending (${pending.length})</h3>`;
      container.innerHTML += pending.map(r => renderLeaveCard(r)).join('');
    }

    if (reviewed.length > 0) {
      container.innerHTML += `<h3 style="color:#888;font-family:'Playfair Display',serif;margin:24px 0 12px;"><i class="fa-solid fa-clipboard-list"></i> Previously Reviewed</h3>`;
    ...
    container.innerHTML = `
      <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:12px;padding:24px;text-align:center;">
        <div style="font-size:2rem;margin-bottom:8px;"><i class="fa-solid fa-circle-info"></i></div>
...
function renderLeaveCard(r) {
  const typeLabel = { leave: '<i class="fa-solid fa-umbrella-beach"></i> Leave', vacation: '<i class="fa-solid fa-plane"></i> Vacation', overtime: '<i class="fa-regular fa-clock"></i> Overtime' }[r.type] || r.type;
  const statusColor = { pending:'#ff9800', approved:'#28a745', rejected:'#dc3545' }[r.status] || '#666';
  const statusLabel = { pending:'<i class="fa-regular fa-clock"></i> Pending', approved:'<i class="fa-solid fa-circle-check"></i> Approved', rejected:'<i class="fa-solid fa-circle-xmark"></i> Rejected' }[r.status] || r.status;
  ...
  return `
    <div style="background:#fff;border-radius:14px;padding:20px 24px;box-shadow:0 2px 8px rgba(0,0,0,0.07);
      ...
      <div style="margin-top:12px;display:flex;gap:24px;flex-wrap:wrap;font-size:0.85rem;color:#666;">
        <span><i class="fa-solid fa-calendar-days"></i> <strong>From:</strong> ${r.startDate ? new Date(r.startDate).toLocaleDateString('en-PH') : '—'}</span>
        <span><i class="fa-solid fa-calendar-days"></i> <strong>To:</strong> ${r.endDate ? new Date(r.endDate).toLocaleDateString('en-PH') : '—'}</span>
...
async function reviewLeaveRequest(id, decision) {
  try {
    ...
    showNotification(`Request ${decision === 'approved' ? 'approved <i class="fa-solid fa-circle-check"></i>' : 'rejected <i class="fa-solid fa-circle-xmark"></i>'}`, decision === 'approved' ? 'success' : 'error');
    loadLeaveRequests();
  } catch(e) {
